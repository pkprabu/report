document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadState() {
        ...
        savedState.metrics.forEach((d, i) => {
        ...
        ['deployed', 'production', 'followup'].forEach(type => {
            ...
            if (header) header.textContent = savedState.headers[type] || defaultState.headers[type];
            section.style.display = savedState.sections[type] ? 'block' : 'none';
            if (savedState.sections[type]) {
                const tbody = document.getElementById(`${type}-campaigns-tbody`);
                tbody.innerHTML = '';
                savedState[type].forEach((d, i) => tbody.appendChild(createCampaignRow(type, i, d)));
            }
        });
